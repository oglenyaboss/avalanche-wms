...
Подбор товаров для отгрузки в конкретный магазин. Работает **по магазинам (destinations)**, а не по отдельным заказам: сборщик получает пачку задач для одного магазина, обходит полки, сканирует QR каждого товара, затем приносит их в буфер отгрузки этого магазина.

**Ключевое:**
- Сборочный лист привязан к магазину (`destination_id`), а не к конкретному `order_id`.
...
1. **Аллокация** (системная) — система находит все `NEW` заказы для магазина, подбирает `STORED` products по SKU, создаёт `assembly_tasks`.
...
3. **Размещение в буфер магазина** — оператор приходит в зону буфера, сканирует его. Все товары из cart попадают в буфер, статус переходит в `READY_TO_SHIP`. Outbox не создаётся.
...
sequenceDiagram
    ...
    WMS->>PG: SELECT orders WHERE destination_id AND status = 'NEW'
    ...
    WMS-->>SYS: Аллокация завершена: 30 товаров для магазина №5
    ...
    WMS-->>UI: Список задач: 30 товаров, ячейки, порядок обхода
    ...
    loop Для каждого товара
        ...
        WMS-->>UI: Товар подобран ✓ (в cart: 3, всего: 30)
    end
    ...
    WMS-->>UI: Размещено 30 товаров в буфере "Магазин №5" ✓
...
```mermaid
stateDiagram-v2
    NEW --> ALLOCATED : Все products заказа получили ALLOCATED
    ALLOCATED --> ASSEMBLED : Все products заказа — в буфере магазина (READY_TO_SHIP)
```

**Важно:** `orders.status = ASSEMBLED` наступает **когда все products заказа уже в буфере** (после scan-shipping-buffer), а не когда они просто подобраны (ASSEMBLED). До этого заказ остаётся в `ALLOCATED`.
...
| `assembly_tasks` | INSERT | Аллокация | PENDING |
| `assembly_tasks.status` | UPDATE | /pick | PENDING → DONE |
| `orders.status` | UPDATE | Аллокация → сборка | NEW → ALLOCATED → ASSEMBLED |
...
VALUES (
  uuid_generate_v4(),       -- свой event_id для outbox
...
| `/pick`: нет PENDING assembly_task для product | `NO_TASK_FOR_PRODUCT` |
