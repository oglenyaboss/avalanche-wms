...
import type { PickedProduct } from '@/entities/assembly'
import {
  Button,
  Sheet,
  SheetContent,
...
// The "Товары к сборке" side drawer: the running list of products picked into
// the assembly task, paginated 20 per page to match the putaway drafter.
export function DrafterSheet({ open, onOpenChange, cart }: DrafterSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="gap-0">
        <SheetHeader>
          <SheetTitle>Товары к сборке</SheetTitle>
          <SheetDescription>
            SKU ранее отсканированных товаров, добавленных в задачу сборки
          </SheetDescription>
        </SheetHeader>
                ...
                <span className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {product.fromBinCode} · заказ {product.orderNo}
                    </span>
                  </span>
                </span>
