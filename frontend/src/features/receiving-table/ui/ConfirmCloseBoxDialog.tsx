...
interface ConfirmCloseBoxDialogProps {
  open: boolean
  isPending: boolean
  onConfirm: () => void
  onCancel: () => void
...
export function ConfirmCloseBoxDialog({
  open,
  isPending,
  onConfirm,
  onCancel,
}: ConfirmCloseBoxDialogProps) {
  return (
    <AlertDialog
      open={open}
      onOpenChange={(next) => {
    ...
    >
      <AlertDialogContent size="sm">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Вы подтверждаете, что больше нечего принимать из коробки?
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
